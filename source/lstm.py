...
class Network(Model):
    ...
    def __init__(self, dim, batch_norm, dropout, rec_dropout, task,
                 ...
                 depth=1, input_dim=76,output_bias=np.log([0.05]), **kwargs):
        ...
        print("final activation is:", final_activation)
        print("------------------------------------")
        ...
        if target_repl:
            y = TimeDistributed(Dense(num_classes, activation=final_activation,bias_initializer=tf.keras.initializers.Constant(output_bias)),
                                name='seq')(L)
            ...
            y = Dense(num_classes, activation=final_activation,bias_initializer=tf.keras.initializers.Constant(output_bias))(L)
            outputs = [y]
